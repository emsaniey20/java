...
public class AliexpressOfferProductEditAeopAEProduct implements Serializable {
    private static final long serialVersionUID = 1L;
        ...
        /** 资质信息。注意删除商品资质信息会导致商品不合规被下架处罚，请不要删除商品资质信息 */
            ...
            private List<AliexpressOfferProductEditAeopQualificationStructDTO> aeopQualificationStructList;
        ...
        /** 商品多媒体信息，该属性主要包含商品的视频列表 */
            ...
            private AliexpressOfferProductEditAeopAEMultimedia aeopAEMultimedia;
        /** 产品属性，以json格式进行封装后提交。参看aeopAeProductPropertys数据结构。此字段是否必填，需从类目接口getAttributesResultByCateId获取（即获取到的required来判断属性是否必填），该项只输入普通类目属性数据，不可输入sku类目属性。对于类目属性包含子类目属性的情况，此处不确认父属性和子属性，即选择任何属性，都以该对象提交。对于一个属性多个选中值的情况，以多个该对象存放。其中"attrNameId","attrValueId"为整型(Integer), "attrName", "attrValue"为字符串类型(String)。         i).  当设置一些系统属性时，如果类目自定义了一些候选值，只需要提供"attrNameId"和"attrValueId"即可。例如：{"attrNameId":494, "attrValueId":284}。         ii). 当设置一些需要手工输入属性内容时，只需要提供"attrNameId"和"attrValue"即可。例如：{"attrNameId": 1000, "attrValue": "test"}         iii）当设置自定义属性时，需要提供"attrName"和"attrValue"即可。例如: {"attrName": "Color", "attrValue": "red"}         iv） 当设置一个Other属性时，需要提供"attrNameId", "attrValueId", "attrValue"三个参数。例如：{"attrNameId": 1000, "attrValueId": 4, "attrValue": "Other Value"}。 */
            ...
            private List<AliexpressOfferProductEditAeopAeProductProperty> aeopAeProductPropertys;
        /** 列表类型，以json格式来表达。参看aeopAeProductSKUs数据结构。特别提示：新增SKU实际可售库存属性ipmSkuStock，该属性值的合理取值范围为0~999999，如该商品有SKU时，请确保至少有一个SKU是有货状态，也就是ipmSkuStock取值是1~999999，在整个商品纬度库存值的取值范围是1~999999。 */
            ...
            private List<AliexpressOfferProductEditAeopAEProductSKU> aeopAeProductSKUs;
        /** 商品分国家报价的配置，想删除分国家报价需要传指定的参数： configuration_type = delete：{"configurationType":"delete","configurationData":""}，计算之后的分国家报价不能小于 sku价格（sku_price和aeop_s_k_u_national_discount_price）的 50%，计算后的分国家报价不能小于0也不能为"null"，请isv做好前置校验 */
            ...
            private AliexpressOfferProductEditAeopNationalQuoteConfiguration aeopNationalQuoteConfiguration;
        ...
        /** 多语言标题 */
            ...
            private List<AliexpressOfferProductEditSubject> subjectList;
        /** 商品初始发布语种标记 */
                @ApiField("locale")
            private String locale;
        /** 多语言详描 */
                @ApiListField("detail_source_list")
            private List<AliexpressOfferProductEditDetailSource> detailSourceList;
        /** 营销图列表 */
                @ApiListField("market_images")
            private List<AliexpressOfferProductEditMarketImage> marketImages;
        /** 运费模版ID。通过运费接口listFreightTemplate获取。 */
                @ApiField("freight_template_id")
            private Long freightTemplateId;
        /** 模特尺码数据 */
                @ApiListField("mode_size_chart_list")
            private List<AliexpressOfferProductEditAeopAEModeSizeChartDTO> modeSizeChartList;
    ...
    this.extParam = extParam;
        ...
        public List<AliexpressOfferProductEditAeopQualificationStructDTO> getAeopQualificationStructList() {
    return this.aeopQualificationStructList;
    }
    public void setAeopQualificationStructList(List<AliexpressOfferProductEditAeopQualificationStructDTO> aeopQualificationStructList) {
    ...
    this.addWeight = addWeight;
        ...
        public AliexpressOfferProductEditAeopAEMultimedia getAeopAEMultimedia() {
    return this.aeopAEMultimedia;
    }
    public void setAeopAEMultimedia(AliexpressOfferProductEditAeopAEMultimedia aeopAEMultimedia) {
    this.aeopAEMultimedia = aeopAEMultimedia;
    }
        public List<AliexpressOfferProductEditAeopAeProductProperty> getAeopAeProductPropertys() {
    return this.aeopAeProductPropertys;
    }
    public void setAeopAeProductPropertys(List<AliexpressOfferProductEditAeopAeProductProperty> aeopAeProductPropertys) {
    this.aeopAeProductPropertys = aeopAeProductPropertys;
    }
        public List<AliexpressOfferProductEditAeopAEProductSKU> getAeopAeProductSKUs() {
    return this.aeopAeProductSKUs;
    }
    public void setAeopAeProductSKUs(List<AliexpressOfferProductEditAeopAEProductSKU> aeopAeProductSKUs) {
    this.aeopAeProductSKUs = aeopAeProductSKUs;
    }
        public AliexpressOfferProductEditAeopNationalQuoteConfiguration getAeopNationalQuoteConfiguration() {
    return this.aeopNationalQuoteConfiguration;
    }
    public void setAeopNationalQuoteConfiguration(AliexpressOfferProductEditAeopNationalQuoteConfiguration aeopNationalQuoteConfiguration) {
    ...
    this.categoryId = categoryId;
        ...
        public List<AliexpressOfferProductEditSubject> getSubjectList() {
    return this.subjectList;
    }
    public void setSubjectList(List<AliexpressOfferProductEditSubject> subjectList) {
    ...
    this.locale = locale;
        ...
        public List<AliexpressOfferProductEditDetailSource> getDetailSourceList() {
    return this.detailSourceList;
    }
    public void setDetailSourceList(List<AliexpressOfferProductEditDetailSource> detailSourceList) {
    this.detailSourceList = detailSourceList;
    }
        public List<AliexpressOfferProductEditMarketImage> getMarketImages() {
    return this.marketImages;
    }
    public void setMarketImages(List<AliexpressOfferProductEditMarketImage> marketImages) {
    ...
    this.freightTemplateId = freightTemplateId;
        ...
        public List<AliexpressOfferProductEditAeopAEModeSizeChartDTO> getModeSizeChartList() {
    return this.modeSizeChartList;
    }
    public void setModeSizeChartList(List<AliexpressOfferProductEditAeopAEModeSizeChartDTO> modeSizeChartList) {
